<section class="section" id="top-radio">
    <div class="p-3">
        <hr>
        <div class="tab-content tab-content-carousel">
            <div class="text-center mb-5">
                <h1 class="mb-3 ff-secondary fw-bold lh-base">Be One Of Our <span class="text-danger">TOP RADIO</span></h1>
            </div>
            <div class="owl-carousel owl-full carousel-equal-height carousel-with-shadow" data-toggle="owl" 
                data-owl-options='{
                    "nav": true, 
                    "dots": true,
                    "lazyLoad": true,
                    "margin": 20,
                    "loop": false,
                    "responsive": {
                        "0": { "items":2 },
                        "600": { "items":3 },
                        "792": { "items":4 },
                        "1040": { "items":5 },
                        "1200": { "items":6 },
                        "1500": { "items":7 },
                        "1800": { "items":9 }
                    }
                }'>
                @foreach($topRadios as $radio)
                    <div class="card-poster radio-card-1">
                        <button type="button" class="btn btn-none p-0" 
                            onclick="window.location.href='{{ route('listener.home') }}?radioId={{ $radio->id }}&radioType={{ $radio->type }}'">
                            <div class="player-icon" style="overflow: hidden;">
                                <img src="{{ $radio->profile->logo 
                                            ? asset('storage/' . $radio->profile->logo) 
                                            : asset('assets/logo/mradiofy-logo.png') }}" 
                                     class="img-fluid" width="50" height="50" alt="Logo">
                            </div>
                        </button>
                        <div class="d-flex justify-content-start">
                            <div>
                                <a wire:navigate href="{{ route('listener.radio',['slug' => $radio->radio_name_slug])}}" rel="noopener noreferrer">
                                    <p class="m-0"><b>{{ $radio->radio_name }}</b></p>
                                </a>
                            </div>
                            <div>
                                @if ($radio->verified)
                                    <img src="{{ asset('/assets/img/verify2.png') }}" alt="" width="15" height="15" class="mx-1"
                                         style="max-width:15px; max-height:15px">
                                @endif
                            </div>
                        </div>
                        <p class="m-0">{{ number_format($radio->listeners_count) }} Listeners</p>
                    </div>
                @endforeach              
            </div><!-- End .owl-carousel -->
        </div><!-- End .tab-content -->
    </div><!-- End .container -->
    <script>
        // You may keep this function if needed; however, the inline onclick already does the navigation.
        function switchRadio(radioId, radioType) {
            window.location.href = `{{ route('listener.home') }}?radioId=${radioId}&radioType=${radioType}`;
        }
    </script>
</section>
