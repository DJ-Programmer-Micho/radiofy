                            ...
                            <div class="position-relative banner">
                              ...
                              <div class="position-absolute top-0 start-0 w-100 h-100" style="background-color: rgba(0, 0, 0, 0.4);"></div> 
                                <div class="position-absolute bottom-0 start-0 p-3 text-white">
                                    ...
                                    <h1>{{ $radioName }}</h1>
                                    <p class="m-0" data-toggle="tooltip" data-placement="top" title="External Radio"><i class="fa-brands fa-connectdevelop"></i></p>
                                </div>
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    ...
                                    <h1>{{ $radioName }}</h1>
                                    <p><i class="fa-brands fa-connectdevelop"></i></p>
...
<script>
    ...
    const logoServerOptions = {
        process: {
            url: '{{ route("filepond.upload.external-logo.temp") }}',
            method: 'POST',
        ...
        revert: (uniqueFileId, load, error) => {
            fetch('{{ route("filepond.revert.external-logo.temp") }}', {
                method: 'DELETE',
                headers: {
    ...
    const bannerServerOptions = {
        process: {
            url: '{{ route("filepond.upload.external-banner.temp") }}',
            method: 'POST',
            headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
            onload: (response) => {
                const cleanedResponse = response.replace(/^"|"$/g, '');
        ...
        revert: (uniqueFileId, load, error) => {
            fetch('{{ route("filepond.revert.external-banner.temp") }}', {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
