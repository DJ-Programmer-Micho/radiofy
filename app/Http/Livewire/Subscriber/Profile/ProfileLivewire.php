...
class ProfileLivewire extends Component
{
    ...
    public function mount()
    {
        ...
        $this->subscriber = auth()->guard('subscriber')->user();

        // Initialize fields from listener profile if available
        $profile = $this->subscriber->subscriber_profile;
        if ($profile) {
        ...
        $this->username = $this->subscriber->username;
    }
    ...
    public function updateProfile()
    {
        ...
        $validatedData = $this->validate([
            ...
            'email'        => ['required', 'email', Rule::unique('listeners')->ignore($this->subscriber->id)],
            'username'     => ['required', 'string', Rule::unique('listeners')->ignore($this->subscriber->id)],
        ...
        $this->subscriber->email = $validatedData['email'];
        $this->subscriber->username = $validatedData['username'];
        ...
        if ($this->avatar) {
            ...
            $destinationDir = storage_path('app/public/subscriber-avatar');
        ...
        if ($this->subscriber->subscriber_profile) {
            $this->subscriber->subscriber_profile->update($profileData);
        } else {
            $this->subscriber->subscriber_profile()->create($profileData);
        }
    ...
    {
        return view('subscriber.pages.profile.profile', [
